...
;(function Preview_Elements_Generic_Scope() {
    ...
    var Preview_Elements_Generic = new Class({
        ...
        animate: function(pElement, pAnimation, pProps) {
            if (pAnimation && pAnimation.easing === UXPAnimationsMap.easing.LINEAR.value && pAnimation.show === '') {
              pAnimation.duration = 0;
            }

            pAnimation = _.defaults(pAnimation || {}, {
                easing: UXPAnimationsMap.easing.LINEAR.value,
                easingType: UXPAnimationsMap.easingType.POWER0.value,
                duration: 0
            });

            if (pAnimation.style && Object.keys(UXPAnimationsMap.legacyEasing).indexOf(pAnimation.style) > -1) {
                pAnimation.duration = pAnimation.time || pAnimation.duration;
                pAnimation.easing = UXPAnimationsMap.legacyEasing[pAnimation.style].easing;
                pAnimation.easingType = UXPAnimationsMap.legacyEasing[pAnimation.style].easingType;
            }
            ...
            var ease;
            if (pAnimation.easingType === UXPAnimationsMap.easingType.ELASTIC.value) {
                var springness = pAnimation.springness / Math.pow(10, (String(pAnimation.springness).length - 2) * 2 + 1);
                ease = window[pAnimation.easingType][pAnimation.easing].config(pAnimation.amplitude / 100, springness);
            } else {
              ease = window[pAnimation.easingType][pAnimation.easing];
            }
